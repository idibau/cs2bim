...
IFC entities. For every configured feature type, a set of instances is created. Currently, three different kinds of
...
Extrusions are feature types that are generated by extruding 2D areas along a polyline. Four different cross sections
types (CIRCLE, EGG, RECTANGLE, POLYGON) and two different extrusion types (POLYLINE (horizontal), LINE (vertical)) are
supported. Every
...
input WKT string of the application. It is expected that this input parameter is used to select the relevant feature
type instances for the request. Each result row is converted into an IFC instance. The required return columns
differ from feature type to feature type.

- Projections: Expects a column with the name **wkt**[2d_wkt_polygon] that represents the area to be projected.
...
- Extrusions: Extrusions are a bit more complicated, because there are different cross sections and extrusion types that
  ...
  cross section and extrusion type. The following table shows the valid values for these columns and the additional
  required columns for each.

  ### extrusion_type
  | Type     | Required Columns                                           |
  |:---------|:-----------------------------------------------------------|
  | POLYLINE | **polyline**[3d_wkt_line_string]                           |
  | LINE     | **start_point**[3d_wkt_point], **end_point**[3d_wkt_point] |

  ### cross_section
  | Type      | Required Columns                                          |
  |:----------|:----------------------------------------------------------|
  | CIRCLE    | **width**[number in meters]                               |
  | EGG       | **width**[number in meters]                               |
  | RECTANGLE | **width**[number in meters], **height**[number in meters] |
  | POLYGON   | **area**[2d_wkt_polygon]                                  |

Additional columns can be included to provide values that can
