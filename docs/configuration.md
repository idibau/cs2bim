...
Extrusions are feature types that are generated by extruding 2D areas along a polyline. Five different cross sections
types (CIRCLE, EGG, RECTANGLE, POLYGON_GLOBAL, POLYGON_LOCAL) and three different extrusion types (POLYLINE (
horizontal), SURFACE (vertical) and POINT (vertical)) are supported.
...
- Extrusions: Extrusions are a bit more complicated, because there are different cross-sections and extrusion types that
  ...
  [[extrusion_type](../src/core/ifc/model/extrusion/extrusion_type.py)] are always mandatory and used to identify the cross-section and extrusion type. The following 
