        ...
        for feature_class_key, elements in model.buildings.items():
            ...
            for element in elements:
                ...
                for building_part in element.building_parts:
                    vertex_dict = {}
                    faces = []
                    for polygon in building_part.points:
                        vertices = []
                    ...
                    if building_part.entity_type == ElementEntityType.IFC_GEOGRAPHIC_ELEMENT:
                        ...
                        raise Exception(
                            f"builing step for feature class entity type {building_part.entity_type.name} not implemented"
                        )

                    for attribute, value in building_part.attributes.items():
                        if hasattr(ifc_element, attribute):
                            setattr(ifc_element, attribute, value)

                    for property_set in building_part.property_sets.values():
                        property_entites = []
                ...
                for attribute, value in element.attributes.items():
                    if hasattr(ifc_building, attribute):
                        setattr(ifc_building, attribute, value)

                for property_set in element.property_sets.values():
                    property_entites = []
                    for key, value in property_set.properties.items():
                        property_entites.append(add_ifc_property_single_value(ifc_file, key, value))
                    add_ifc_property_set(ifc_file, property_set.name, property_entites, ifc_building)
            ...
            add_ifc_rel_contained_in_spatial_structure(ifc_file, buildings, ifc_spatial_structures[
                feature_class.spatial_structure.get_key()])
