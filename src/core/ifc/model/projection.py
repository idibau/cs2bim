...
class Projection(FeatureElement):
    ...
    def __init__(self, data: tuple[list[list[float]], list[list[int]]]):
        ...
        for triangle in index_list:
            p1 = Coordinates(*point_list[triangle[0]])
            p2 = Coordinates(*point_list[triangle[1]])
            p3 = Coordinates(*point_list[triangle[2]])
            self.triangles.append((p1, p2, p3))

    def map_to_ifc(self, ifc_file: IfcFile, entity: ProjectionEntity, placement_rel_to: entity_instance,
